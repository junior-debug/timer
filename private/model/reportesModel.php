...
class database
{
  ...
  public function cantidadPosicionMES($servicioP_, $mesP_, $anioP_, $rol_cargo)
  { //echo $servicioP_.'<br>';   
    if ($rol_cargo  == 'ADMIN') {
  ...
  public function consultarxauxiliarAlt($fecha1, $fecha2)
  {
    //echo "<br> SELECT registro.id_usuario, registro.dia, registro.$auxiliar, registro.hora_inicio, registro.hora_fin, users.id_user, users.nombre, users.apellido FROM registro INNER JOIN users ON users.id_user = registro.id_usuario WHERE registro.dia BETWEEN '$fecha1' AND '$fecha2' ".'<br><br>';
    //echo "SELECT * FROM registro INNER JOIN users ON users.id_user = registro.id_usuario WHERE registro.dia BETWEEN '$fecha1' AND '$fecha2'";
  ...
  public function consultarxauxiliar($fecha1, $fecha2, $campaign, $cargoUSer)
  {
    //echo "<br> SELECT registro.id_usuario, registro.dia, registro.$auxiliar, registro.hora_inicio, registro.hora_fin, users.id_user, users.nombre, users.apellido FROM registro INNER JOIN users ON users.id_user = registro.id_usuario WHERE registro.dia BETWEEN '$fecha1' AND '$fecha2' ".'<br><br>';
    //echo "SELECT * FROM registro INNER JOIN users ON users.id_user = registro.id_usuario WHERE registro.dia BETWEEN '$fecha1' AND '$fecha2'";
    if ($cargoUSer == 'CLIENTE') {
      $sql = $this->db->query("SELECT * FROM registro INNER JOIN datos_empleados ON datos_empleados.id_datos_empleados = registro.id_usuario INNER JOIN campanas ON campanas.abrev_campana = datos_empleados.campana WHERE registro.dia BETWEEN '$fecha1' AND '$fecha2' AND datos_empleados.servicio = '$campaign'");
      if ($this->db->rows($sql) > 0) {
        while ($data = $this->db->recorrer($sql)) {
          $respuesta[] = $data;
        }
  ...
  { //(cargo='SUPERVISOR' OR cargo='COORDINADOR')
  ...
  public function busquedaAgnte($busqueda, $campaign, $cargoUSer)
  {
    //echo "SELECT * FROM users WHERE nombre LIKE '%$busqueda%' OR apellido LIKE '%$busqueda%';".'<br><br>';
    if ($cargoUSer == 'CLIENTE') {
      $sql = $this->db->query("SELECT * FROM datos_empleados WHERE (estatus = 1) AND servicio = '$campaign' AND (cargo = 'OPERADOR') AND (nombre LIKE '%$busqueda%' OR apellido LIKE '%$busqueda%');");
      if ($this->db->rows($sql) > 0) {
        while ($data = $this->db->recorrer($sql)) {
          $respuesta[] = $data;
        }
  ...
  public function cargosList($valorCargo)
  { //echo "SELECT * FROM cargos WHERE estatus = 1 ".$valorCargo;
  ...
  public function supervisoresList()
  { //echo "  ";
