...
$(document).ready(function(){
	...
	$("#botonDescargarExcell").attr({ href:'public/archivos/FORMATO_DE_CARGA_ServiCampana_.xlsx'});
	...
	$('#btn-register').click(function(){ 
	    ...
	    if (nombre == "" || apellido == "" || cedula == "" || genero == null || cargo == null || fecha_ingreso == "" || rotacion == null || turno == null || users == ""){
	    		...
	    		if ( cargo == 'OPERADOR') {
	    			...
	    			if (supervisor == null || supervisor == '' || supervisor == 0 || servicio == null || servicio == '' || servicio == 0 || campana == null || campana == '' || campana == 0) {
					    alert(' El operador debe estar asociado a un Supervisor y/o Servicio. Revisar');
					    //supervisor = 0;
					}else{
					  /* 	alert('OPERADORRR  scripttt nombre: ' + nombre+ ' ==> apellido: '+ apellido+ ' ==> cedula: '+ cedula+ ' ==> genero: '+ genero+ ' ==> cargo: '+cargo+
						...
						*/
						$.ajax({
							...
							dataType: "json",
							data:{nombre: nombre, apellido: apellido, cedula: cedula, genero:genero, cargo: cargo, fecha_ingreso: fecha_ingreso,
							servicio: partes_[1], campana: campana, rotacion:rotacion, turno: turno, supervisor: supervisor, users: users, passs:passs, radioSuper: 1},
	    		...
	    		}else if ( cargo == 'ANALISTA' || cargo == 'COORDINADOR' ||cargo == 'CLIENTE' ) {
	    			servicio   = $('#servicio_index').val();
					...
					$.ajax({
						...
						dataType: "json",
						data:{nombre: nombre, apellido: apellido, cedula: cedula, genero:genero, cargo: cargo, fecha_ingreso: fecha_ingreso,
						servicio: 'ADMIN', campana: 'ADMIN', rotacion:'ADMIN', turno: 'ADMIN', supervisor: supervisor, users: users, passs:passs, radioSuper: 1},
							success:function(datos){ 
								//alert(datos.response)
								if(datos.response == 'true'){
									alert('REGISTRO EXITOSO');
	    			...
	    			supervisor = 0;
	    			if( SelectRadio  == "" || SelectRadio  == 0  || SelectRadio  == null  || SelectRadio  == undefined){
	    				alert(' Deben selecionar si esta asociado a un servicio o varios -_-')

	    			}else{
	    				if (SelectRadio == 1) { // un servicio
		    				servicio    	= $('#servicio_index').val();
		    				campana    		= $('#campana_index').val();
		    				var partes_ 	= servicio.split('?/?');
					            	...
					            	if (servicsArra.length == campansArra.length) {
										...
										*/
										...
										$.ajax({
											...
											dataType: "json",
											data:{nombre: nombre, apellido: apellido, cedula: cedula, genero:genero, cargo: cargo, fecha_ingreso: fecha_ingreso,
											servicio: servicsArra, campana: campansArra, rotacion:rotacion, turno: turno, supervisor: supervisor, users: users, passs:passs, radioSuper: 2},
											success:function(datos){ 
												//alert(datos)
												if(datos.response == 'true'){
													alert('REGISTRO EXITOSO');
					            		...
					            		alert('Existe diferencias de cantidad entre servicios y campañas. Verificar.')
					            	}
	...
	$('#updateEdit').click(function(){  //OJOPOPOPOPOPOPOPOPOPOPOP, VALIDAR SI LA CEDULA A EDITAR NO SE REPITA
		var campanaEdit_ = 0;
		...
		if (campana_index == undefined || campana_index == null || campana_index == '0' || campana_index == 0) {

			campanaEdit_ = campanaEdit

		
		}else if (campanaEdit == null || campanaEdit == undefined || campanaEdit == '0'  || campanaEdit == 0) {
...
// alert(id_datos_empleados+ ' ==> '+ nombreEdit+ ' ==> '+apellidoEdit+ ' ==> '+cedulaEdit+ ' ==> '+ genero_edit+ ' ==> '+cargoEdit + ' ==> servicioEdit==>-* '+ servicioEdit+ ' ==> '+campanaEdit_+ ' ==> '+ rotacionEdit+ ' ==> '+ turnoEdit+ ' ==> '+ supervisorEdit+ ' ==> '+ supervisor_index + ' ==> ' + $('#valorCambioServEdit').val() + ' campana_index  ==> ' +campana_index + ' campanaEdit==> ' +campanaEdit )
		...
		if (nombreEdit == "" || apellidoEdit == "" || cedulaEdit == "" || cargoEdit == "" || servicioEdit == "" || servicioEdit == undefined || rotacionEdit == "" || turnoEdit == "")  {
			alert('Campos vacíos. Revisar.')

		}else{ 
  
			if ( cargoActualEdit == 'SUPERVISOR') {	
				if ( campanaEdit_ == undefined || campanaEdit_ == null || campanaEdit_ == '0' || campanaEdit_ == 0 ) {
					alert('Campos vacíos. Revisar.')
					...
					if ( cargoActualEdit != cargoEdit) {
						if (supervisorEditNuevo != 0) {
								...
								$.ajax({
									...
									dataType: "json",
									data:{id_datos_empleados: id_datos_empleados, nombreEdit: nombreEdit, apellidoEdit: apellidoEdit, cedulaEdit: cedulaEdit,
									      	genero_edit:genero_edit,cargoEdit: cargoEdit,servicioEdit: servicioEdit,campanaEdit_:campanaEdit_, rotacionEdit:rotacionEdit, turnoEdit:turnoEdit,
									      	supervisorEdit:supervisorEdit, valorEdit: 2, supervisorEditNuevo: supervisorEditNuevo, IdCargoActualEdit: IdCargoActualEdit},
									success:function(datos){ 
										//alert(datos.response)
										if(datos.response == 'true'){
								...
								alert( 'hubo cambio de cargo, por favor seleccionar el nuevo supervisor, de lo contrario no guardará NADA' )
						}
						...
						$.ajax({
							...
							data:{id_datos_empleados: id_datos_empleados, nombreEdit: nombreEdit, apellidoEdit: apellidoEdit, cedulaEdit: cedulaEdit,
							      	genero_edit:genero_edit,cargoEdit: cargoEdit,servicioEdit: servicioEdit,campanaEdit_:campanaEdit_, rotacionEdit:rotacionEdit, turnoEdit:turnoEdit,
							      	supervisorEdit:supervisorEdit,  valorEdit: 1, supervisorEditNuevo: 0, IdCargoActualEdit: IdCargoActualEdit},
							success:function(datos){ 
								//alert(datos.response)
								if(datos.response == 'true'){
									alert('USUARIO ACTUALIZADO');
	...
	$('#updateEditReincorpo').click(function(){
		...
		if (nombreEdit == "" || apellidoEdit == "" || cedulaEdit == "" || cargoEdit == "" || servicioEdit == "" || rotacionEdit == "" || turnoEdit == "")  {
			alert('Campos vacíos. Revisar.')
			...
			$.ajax({
				...
				dataType: "json",
				data:{id_datos_empleados: id_datos_empleados, nombreEdit: nombreEdit, apellidoEdit: apellidoEdit, cedulaEdit: cedulaEdit,
				      	genero_edit:genero_edit,cargoEdit: cargoEdit,fechaIngreEdit, servicioEdit: servicioEdit,campanaEdit_:campanaEdit_,
				      	rotacionEdit:rotacionEdit, turnoEdit:turnoEdit, supervisorEdit:supervisorEdit, usersEdit:usersEdit},
	...
	$('#btn-DeleteUser').click(function(){ 
		...
		if ($('#razonDelete').val() == null || $('#razonDelete').val() == "" || $('#observacionesDelete').val() == "" ) {
...
function validar_clave_(clave/*,$error_clave*/){
	...
	if ( $('#list1_').val() == 12 &&  $('#list2_').val() == 12 &&  $('#list3_').val() == 12 &&  $('#list4_').val() == 12 &&  $('#list5_').val() == 12) {
...
function button_modalUpdatePasswor(e){//alert(e)
	$.ajax({
		type:'POST',
...
function ReincorpoUser(){
	...
	$(location).attr('href','?view=usuarios&mode=FormReincorUser&id='+empleado);
}
/**/
function mayus(e) {
...
function button_modalDelete(e){
	$('#razonDelete').val("");   $('#observacionesDelete').val("");
	$.ajax({
		type:'POST',
		url:'?view=usuarios&mode=datosUser',
		dataType: "json",
...
function cheboxServe(e){
	...
	$("form .servMuch").each(function() { this.selectedIndex = 0 });
	$("form .campMuch").each(function() { this.selectedIndex = 0 });
	$('#resultSelectRadio').val("");
	$('#agregarBloque').val("");

	if ( e == 'cheboxS1' ) {                      //  variosServiceUNOOOOOOO
			//alert(' cheboxS1 ');
			$('#resultSelectRadio').val(1);
			$('#cuadroVariosServicios').hide();
			$('#servicio_index').val("");       $('#campana_index').val(""); //esto es de un servicio 		
			$('#servicio_index').removeAttr('disabled','disabled');
			$('#cuadroUnServicios').show();
			$('#div_bloqueCampana').show();

	}else{                                        //  variosServiceMUUUUCHOOOOSS
			//alert(' cheboxS2 ');
			$('#resultSelectRadio').val(2);
			$('#servicio_index').val("");       $('#campana_index').val("");
...
function selectOnchangeEdit_(e){ 
	...
	$.ajax({
		...
		url:'?view=usuarios&mode=selectCampanas',
		dataType: "json",
		data:{id_servicio: partes_[0], nombreService: $('#name_servicioEdit').val() },
		success:function(datos){ 
